...
#climo_string = 'climo_files/cmean_1d.1959VALIDMVALIDD'
...
gfsanl_string = 'gfs.%Y%m%d/gfs.t%Hz.pgrb2.0p50.f000'
rapanl_string = 'rap.%Y%m%d/rap.t%Hz.awip32f00.grib2'

# ~~~~~~~
# Valid time
# Can be a single datetime or a tuple of datetimes. In the latter case, plots will be a mean value of the period requested
# ~~~~~~~
#valids = [(datetime(2021,12,7,00,0,0), datetime(2021,12,12,00,0,0))]#,
          #(datetime(2021,1,18,12,0,0), datetime(2021,1,25,12,0,0))] # inclusive
valids = [datetime(2021,12,16,00,0,0)]
# ~~~~~~~
# step between each valid in the period requested (used to compute the period average) 
...
# ~~~~~~~
# If true, temperature maps (not temperature anomaly maps) will include contoured 32 F line
# ~~~~~~~
zero_contour=True
...
# Given a string, output is either the map projection and figure objects or lonrange and latrange tuples that define the desired domain
   ...
   if model_string[-3:] == '.nc':
      ...
      for f, fpath in enumerate(fpaths):
         ...
         with xr.open_dataset(fpath, engine='cfgrib', backend_kwargs=dict(
            indexpath='',
            filter_by_keys={'typeOfLevel':type_of_level,'shortName':short_name})) as ds_t:
...
def plot_anom(num, model, lons, lats, data_tmp, valid, plevel, init=None, domain=None, lonrange=None, latrange=None, parallels=None, merid=None):
   print(f'Plot {num} ... {plevel} hPa Temperature Anomaly plot for {str(model).upper()}')
   print('========================================')
   ...
   MEG_temp_anom_colors = [
      ...
      '#c1e7f8', # -6.0 - -4.0 F 
      '#e6f5fb', # -4.0 - -2.0 F 
      '#A9A9A9', # -2.0 - 2.00 F #ffffff originally white 
      '#fdf3c5', # 2.00 - 4.00 F 
      '#fdd778', # 4.00 - 6.00 F 
      '#fdaa31', # 6.00 - 8.00 F 
      '#f77c2b', # 8.00 - 10.0 F 
   ...
   cmap = colors.ListedColormap(MEG_temp_anom_colors)
   norm = colors.BoundaryNorm(clevs, cmap.N, clip=False)

   #plot_tmp = ax.pcolormesh(lons, lats, data_tmp, shading='flat', cmap=cmap, zorder=1, transform=ccrs.PlateCarree(), norm=norm)
   #Use contourf when using finer_grid
   plot_tmp = ax.contourf(lons, lats, data_tmp, shading='flat', levels=clevs, cmap=cmap, zorder=1, transform=ccrs.PlateCarree(), norm=norm)
   print('Plotted')
   ...
   if str(plevel).upper() == '2M':
      title_string_right = f'2-m T Anomaly (F)'
   else:
      title_string_right = f'{plevel}-hPa T Anomaly (F)'
   ...
   cb = plt.colorbar(plot_tmp, orientation='vertical', cax=cax, cmap=cmap, norm=norm, boundaries=clevs, spacing='uniform', ticks=cbar_ticks, drawedges=True)
   ...
   if init:
      if isinstance(valid, tuple):
         save_string = f'{model}_{str(plevel).upper()}TEMP_ANOMALY_{domain}.'+init.strftime('init_%Y%m%d%H.')+valid[0].strftime('valid_%Y%m%d%H')+valid[1].strftime('_%Y%m%d%H.png')
      else:
         save_string = f'{model}_{str(plevel).upper()}TEMP_ANOMALY_{domain}.'+init.strftime('init_%Y%m%d%H.')+valid.strftime('valid_%Y%m%d%H.png')
   else:
      if isinstance(valid, tuple):
         save_string = f'{model}_{str(plevel).upper()}TEMP_ANOMALY_{domain}.'+valid[0].strftime('valid_%Y%m%d%H')+valid[1].strftime('_%Y%m%d%H.png')
      else:
         save_string = f'{model}_{str(plevel).upper()}TEMP_ANOMALY_{domain}.'+valid.strftime('valid_%Y%m%d%H.png')
...
def plot_temp(num, model, lons, lats, data_tmp, valid, plevel, init=None, domain=None, lonrange=None, latrange=None, parallels=None, merid=None, zero_contour=False):
   print(f'Plot {num} ... {plevel} hPa Temperature plot for {str(model).upper()}')
   print('========================================')
   print('Prepping Basemap')
   ...
   # Set freezing contour
   contour_ints = [32.]

   #plot_tmp = ax.pcolormesh(lons, lats, data_tmp, shading='flat', cmap=cmap, vmin=-60., vmax=60., zorder=1, transform=ccrs.PlateCarree(), norm=norm)
   #contourf looks better but needs mask
   mask1 = lons < lonrange[0] - 36.
   mask2 = lons > lonrange[1] + 36.
   mask3 = lats > latrange[1] - 0.
   mask4 = lats < latrange[0] + 0.
   mask = mask1+mask2+mask3+mask4
   mdata_tmp = np.ma.MaskedArray(data_tmp,mask=mask)
   plot_tmp = ax.contourf(lons, lats, mdata_tmp, shading='flat', levels=clevs, cmap=cmap, zorder=1, transform=ccrs.PlateCarree(), norm=norm)
    
   if zero_contour:
      # Mask data that are out of bounds
      mask1 = lons < lonrange[0] - 36.
      mask2 = lons > lonrange[1] + 36.
      mask3 = lats > latrange[1] - 1.
      mask4 = lats < latrange[0] + 1.
      mask = mask1+mask2+mask3+mask4
      mdata_tmp = np.ma.MaskedArray(data_tmp,mask=mask)
      plot_zero = ax.contour(lons, lats, mdata_tmp, levels=contour_ints, colors='white', linewidths=1.5, zorder=9, transform=ccrs.PlateCarree())
   ...
   cb.dividers.set_linewidth(2)
   cb.ax.tick_params(labelsize=8, labelright=True, labelleft=False, right=False)
   cb.ax.set_yticklabels(['-60','-52','-44','-36','-28','-20','-12','-4','4','12','20','28','36','44','52','60',''])
   cax.hlines([0, 1], 0, 1, colors='black', linewidth=4)

   if init:
      if isinstance(valid, tuple):
         save_string = f'{model}_{str(plevel).upper()}TEMP_{domain}.'+init.strftime('init_%Y%m%d%H.')+valid[0].strftime('valid_%Y%m%d%H')+valid[1].strftime('_%Y%m%d%H.png')
      else:
         save_string = f'{model}_{str(plevel).upper()}TEMP_{domain}.'+init.strftime('init_%Y%m%d%H.')+valid.strftime('valid_%Y%m%d%H.png')
   else:
      if isinstance(valid, tuple):
         save_string = f'{model}_{str(plevel).upper()}TEMP_{domain}.'+valid[0].strftime('valid_%Y%m%d%H')+valid[1].strftime('_%Y%m%d%H.png')
      else:
         save_string = f'{model}_{str(plevel).upper()}TEMP_{domain}.'+valid.strftime('valid_%Y%m%d%H.png')
   save_path = os.path.join(SAVE_DIR, save_string)
   plt.savefig(save_path, bbox_inches='tight')
   print(f'Plot {num} saved successfully in {save_path}')
   plt.close(num)
      ...
      for model in models:
         ...
         for valid in valids:
            for plevel in plevels:
               ...
               try: 
                  fcst_tmp, fcst_lats, fcst_lons = upper_data(init, valid, str(model).upper(), fcst_string, plevel, step=step)
                  obs_tmp, obs_y, obs_x = upper_data(init, valid,  str(analysis).upper()+'_ANALYSIS', analysis_string, plevel, step=step, use_valid=use_valid)
                  climo_tmp, climo_lats, climo_lons = upper_data(init, valid, 'CLIMO', climo_string, plevel, use_valid=use_valid, step=step)
                  fcst_lons, fcst_lats = np.meshgrid(fcst_lons, fcst_lats)
                  #Comment out line below for RAP; meshgrid not needed
                  obs_lons, obs_lats = np.meshgrid(obs_x, obs_y)
                  climo_lons, climo_lats = np.meshgrid(climo_lons, climo_lats)
                  climo_interp = diff_data(obs_lons, obs_lats, climo_lons, climo_lats, climo_tmp)
                  #climo_interp = diff_data(obs_x, obs_y, climo_lons, climo_lats, climo_tmp)
                  fcst_interp = diff_data(obs_lons, obs_lats, fcst_lons, fcst_lats, fcst_tmp)
                  #fcst_interp = diff_data(obs_x, obs_y, fcst_lons, fcst_lats, fcst_tmp)
                  ...
                  #anom_lons, anom_lats = (obs_x, obs_y)
                  if finer_grid and str(analysis).upper() == 'GFS':
                     if obs_y[0] > obs_y[-1]:
                  ...
                  elif finer_grid and str(analysis).upper() == 'RAP':
                     if (obs_y[0] > obs_y[-1]).all():
                        lats_fine = np.arange(obs_y[0], obs_y[-1], -1*finer_grid)
                     else:
                        lats_fine = np.concatenate((np.arange((obs_y[0]).all(), np.min(obs_y), -1*finer_grid), np.arange(np.max(obs_y), (obs_y[-1]).all(), -1*finer_grid)))
                     if (obs_x[0] < obs_x[-1]).all():
                        lons_fine = np.arange(obs_x[0], obs_x[-1], finer_grid)
                     else:
                        lons_fine = np.concatenate((np.arange((obs_x[0]).all(), np.max(obs_x), finer_grid), np.arange(np.min(obs_x), (obs_x[-1]).all(), finer_grid)))
                     lons_fine, lats_fine = np.meshgrid(lons_fine, lats_fine)
                     fcst_anom = diff_data(lons_fine, lats_fine, obs_x, obs_y, fcst_anom)
                     obs_anom = diff_data(lons_fine, lats_fine, obs_x, obs_y, obs_anom)
                     anom_lons, anom_lats = (lons_fine, lats_fine)
                  if str(domain).upper() == 'OTHER':
                     plot_temp(num, str(model).upper(), fcst_lons[1:-1,1:-1], fcst_lats[1:-1,1:-1], fcst_tmp[1:-1,1:-1], valid, plevel, init=init, domain=domain, lonrange=lonrange, latrange=latrange, parallels=parallels, merid=merid, zero_contour=zero_contour)
                  else:
                     plot_temp(num, str(model).upper(), fcst_lons[1:-1,1:-1], fcst_lats[1:-1,1:-1], fcst_tmp[1:-1,1:-1], valid, plevel, init=init, domain=domain, zero_contour=zero_contour)
                  num+=1
                  if str(domain).upper() == 'OTHER':
                     plot_temp(num, str(analysis).upper()+'_ANALYSIS', obs_lons[1:-1,1:-1], obs_lats[1:-1,1:-1], obs_tmp[1:-1,1:-1], valid, plevel, init=None, domain=domain, lonrange=lonrange, latrange=latrange, parallels=parallels, merid=merid, zero_contour=zero_contour)
                     #Include line below if using RAP
                     #plot_temp(num, str(analysis).upper()+'_ANALYSIS', obs_x[1:-1,1:-1], obs_y[1:-1,1:-1], obs_tmp[1:-1,1:-1], valid, plevel, init=None, domain=domain, lonrange=lonrange, latrange=latrange, parallels=parallels, merid=merid, zero_contour=zero_contour)
                  else:
                     plot_temp(num, str(analysis).upper()+'_ANALYSIS', obs_lons[1:-1,1:-1], obs_lats[1:-1,1:-1], obs_tmp[1:-1,1:-1], valid, plevel, init=None, domain=domain, zero_contour=zero_contour)
                     #plot_temp(num, str(analysis).upper()+'_ANALYSIS', obs_x[1:-1,1:-1], obs_y[1:-1,1:-1], obs_tmp[1:-1,1:-1], valid, plevel, init=None, domain=domain, zero_contour=zero_contour)
                  num+=1
                  if str(domain).upper() == 'OTHER':
                     plot_temp(num, 'CLIMO', climo_lons[1:-1,1:-1], climo_lats[1:-1,1:-1], climo_tmp[1:-1,1:-1], valid, plevel, init=None, domain=domain, lonrange=lonrange, latrange=latrange, parallels=parallels, merid=merid, zero_contour=zero_contour)
                  else:
                     plot_temp(num, 'CLIMO', climo_lons[1:-1,1:-1], climo_lats[1:-1,1:-1], climo_tmp[1:-1,1:-1], valid, plevel, init=None, domain=domain, zero_contour=zero_contour)
                  num+=1
                  if str(domain).upper() == 'OTHER':
