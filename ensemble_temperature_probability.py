...
# Description: Plot temperature probability outlooks using ensembles grib files
# and analysis grid values outlooks defined as probabilities of above or below 
# normal
# Author: Marcel Caron
...
# and use PP in place of ensemble mem, FF in place of lead time, STAT in place 
# of climo statistic (mean, stdv ...)
# ~~~~~~~
gefs_string = 'common/gefs.%Y%m%d/gePP.t%Hz.pgrb2a.0p50.fFF' 
climo_string = 'noscrub/climo_files/STAT_%m%d' 
...
def climo_lower_data(init, climo_string, fstart, fend):
   ...
   for f, fvalid in enumerate(fvalids):
      if valid_hours[f] in [0,6,12]:
         ...
         # 18Z data are stored in the file named after the following day for 
         # some reason
         use_date = init+td(hours=int(fvalid)+24) 
      fpath_mean = os.path.join(
         DATA_DIR, use_date.strftime(climo_string.replace('STAT','mean'))
      )
      fpath_stdev = os.path.join(
         DATA_DIR, use_date.strftime(climo_string.replace('STAT','stdev'))
      )
      ...
      # 33.333% of values in a normal distribution fall within +/- .43072 
      # standard deviations from the mean (we're assuming normal) .43 is used 
      # in E. Becker and van den Dool, H. (2016) Probabilistic seasonal 
      # forecasts in the North American Multimodel Ensemble: A baseline skill 
      # assessment. J. Climate, 29(8), 3015-26
   ...
   # convert K to F
   data_upper = np.add(
      np.multiply(np.subtract(data_upper, 273.15), 9./5.), 32.
   )
   data_lower = np.add(
      np.multiply(np.subtract(data_lower, 273.15), 9./5.), 32.
   )
...
def ens_lower_data(init, model, model_string, fstart, fend, ens_mem=None, 
                   analysis=False):
   
   print('========================================')
   if ens_mem:
      print(f'Getting and Summing {model} {ens_mem} Member Pressure Data')
   else:
      print(f'Getting and Summing {model} Pressure Data')
   if analysis:
   ...
   if str(model).upper() in ['GEFS']: # global models have 3 leading zeros
      for f, fvalid in enumerate(fvalids):
         if analysis:
            fpath = os.path.join(
               DATA_DIR, 
               (init+td(hours=int(fvalid)))
                  .strftime(model_string)
                  .replace('FF','{0:0=3d}'.format(0))
                  .replace('PP',ens_mem)
            ) 
         else:
            fpath = os.path.join(
               ...
               init.strftime(model_string)
                  .replace('FF','{0:0=3d}'.format(int(fvalid)))
                  .replace('PP',ens_mem)
            ) 
      ...
      for f, fvalid in enumerate(fvalids):
         fpath = os.path.join(
            DATA_DIR, 
            init.strftime(model_string)
               .replace('FF','{0:0=2d}'.format(int(fvalid)))
         )
         with xr.open_dataset(fpath, engine='cfgrib', backend_kwargs=dict(
            indexpath='',
            filter_by_keys={'typeOfLevel':'heightAboveGround'})) as ds:
   ...
   # convert K to F
   data = np.add(np.multiply(np.subtract(data, 273.15), 9./5.), 32.) 
...
def diff_data(anl_lons, anl_lats, model_lons, model_lats, model_data):
   anl_lons, anl_lats, model_lons, model_lats, model_data = [
      np.array(item) for item in [anl_lons, anl_lats, model_lons, model_lats, 
                                  model_data
      ]
   ]
...
def BSS(prob, anal):
   # .33 probability that fcst is in any given tercile (above, below, or 
   # normal), for all locations
   climo = np.divide(np.ones(anal.shape), 3) 
   BS = np.nanmean(np.power(np.subtract(prob, anal), 2))
   # BS of climatological forecast vs obs (climo forecast = 0, i.e, no anomaly)
   BS_ref = np.nanmean(np.power(np.subtract(climo, anal), 2)) 
   BSS = 1 - (BS/BS_ref)
   return BSS

def get_ABN(ensemble_data, clim_upper, clim_lower):
   total = np.array(
      [[len(ensemble_data[:,r,c]) for c, column in enumerate(row)] 
      for r, row in enumerate(ensemble_data[0])]
   )
   above = np.divide(np.sum(ensemble_data>clim_upper, axis=0), total)
   below = np.divide(np.sum(ensemble_data<clim_lower, axis=0), total)
   normal = np.divide(
      np.sum((ensemble_data<clim_lower)+(ensemble_data>clim_upper)==0, axis=0),
      total
   )
...
def plot_temp_probability(num, model, lons, lats, data, fstart, fend, 
                          BSS_under=None, BSS_over=None, BSS_normal=None, 
                          analysis=False, init=None, domain=None, 
                          parallels=None, merid=None, latrange=None, 
                          lonrange=None, figsize=None):
   if analysis:
      print(f'Plot {num} ... Temperature Anomaly Probability for'
            + f' {str(model).upper()} (Analysis)')
   else:
      print(f'Plot {num} ... Temperature Anomaly Probability for' 
            + f'{str(model).upper()}')
   print('========================================')
   ...
   if str(domain).upper() == 'OTHER':
      proj, fig = get_domains(domain=domain, parallels=parallels, merid=merid, 
   ...
   clevs = [-1., -.9, -.8, -.7, -.6, -.5, -.4, -.3333, .3333, .4, .5, .6, .7, 
            .8, .9, 1.]
   n=len(clevs)-1
   vmin, vmax = (-1., 1.)
   MEG_error_colors = [
   ...
   plot_temp_prob = ax.pcolormesh(
      lons, lats, data, shading='flat', cmap=cmap, vmin=vmin, vmax=vmax, 
   ...
   if valid_start.year == valid_end.year:
      if valid_start.month == valid_end.month:
         valid_string = (valid_start.strftime(' valid %HZ %d - ') 
                         + valid_end.strftime('%d %B %Y'))
      else:
         valid_string = (valid_start.strftime(' valid %HZ %d %B - ') 
                         + valid_end.strftime('%d %B %Y'))
   else:
      valid_string = (valid_start.strftime(' valid %HZ %d %B %Y - ') 
                      + valid_end.strftime('%HZ %d %B %Y'))
   if analysis:
      ...
      title_string_left = (f'{model_name} | ' 
                           ...
                           + valid_string)
   title_string_right = r'T Probability (%)' 
   ...
   if not analysis:
      above_text = ax.text(
         x=1.11, y=0.85, s='Above\nNormal\nBSS: {:.2f}'.format(BSS_over), 
         transform=ax.transAxes, horizontalalignment='center', 
         verticalalignment='center', color='black', fontsize=10, zorder=10
      )
      below_text = ax.text(
         x=1.11, y=0.15, s='Below\nNormal\nBSS: {:.2f}'.format(BSS_under), 
         transform=ax.transAxes, horizontalalignment='center', 
         verticalalignment='center', color='black', fontsize=10, zorder=10
      ) 
      normal_text = ax.text(
         x=1.11, y=0.5, s='Near\nNormal\nBSS: {:.2f}'.format(BSS_normal), 
         transform=ax.transAxes, horizontalalignment='center', 
         verticalalignment='center', color='black', fontsize=10, zorder=10
      ) 
   else:
      above_text = ax.text(
         x=1.11, y=0.85, s='Above\nNormal', transform=ax.transAxes, 
   ...
   fig.subplots_adjust(
      left=.05, right=.91, top=.95, bottom=.05, wspace=0.05, hspace=0
   )
   cax = fig.add_axes([.91, .05, .01, .9])
   cbar_ticks = [-1., -.9, -.8, -.7, -.6, -.5, -.4, -.3333, 
                 .3333, .4, .5, .6, .7, .8, .9, 1.]
   cb = plt.colorbar(
      plot_temp_prob, orientation='vertical', cax=cax, cmap=cmap, norm=norm, 
      boundaries=clevs, spacing='uniform', ticks=cbar_ticks, drawedges=True
   )
   cb.dividers.set_color('black')
   cb.dividers.set_linewidth(2)
   cb.ax.tick_params(
      size=0, labelsize=8, labelright=True, labelleft=False, right=False
   )
   cb.ax.set_yticklabels(
      ['100','90','80','70','60','50','40','33',
      '33','40','50','60','70','80','90','100']
   )
   cax.hlines([0, 1], 0, 1, colors='black', linewidth=4)
    
   if analysis:
      save_string = (f'{model}_ANALYSIS_TEMP_ANOM_PROBABILITY_{domain}.'
                     + valid_start.strftime('valid_%Y%m%d%H')
                     + valid_end.strftime('_%Y%m%d%H.png'))
   else:
      save_string = (f'{model}_TEMP_ANOM_PROBABILITY_{domain}.'
...
def main(lon_range=None, lat_range=None):
   ...
   elif str(analysis).upper() == 'GFSV15':
      analysis_string = gfsv15_string
   elif str(analysis).upper() == 'GFSV16':
   ...
   # Each dataset needs to be processed separately because they contain 
   # different precip accumulation step ranges
   num = 0
   # Given init_date
         ...
         for model in models:
               ...
               try: 
                  if str(model).upper() == 'GEFS':
                     ...
                     gefs_ens_mems = [
                        'p{:02d}'.format(num) 
                        for num in np.arange(1, gefs_ens_num+1)
                     ]

                     # Get analysis temperature data
                     if str(analysis).upper() == 'GEFS':
                        use_mem_for_anal = 'ctl'
                     elif str(analysis).upper() in ['GFSV15','GFSV16']:
                        use_mem_for_anal = 'gfs'
                     else:
                        raise OSError(
                           f'Script does not support using {analysis} as the'
                           + f' analysis dataset, use \'gfsv16\', \'gfsv15\', '
                           + f'or \'gefs\''
                        )

                     anal_temp, anal_y, anal_x = ens_lower_data(
                        init, str(analysis).upper(), analysis_string, fstart, 
                        fend, ens_mem='gfs', analysis=True
                     )
                     (climo_upper_temp, climo_lower_temp, climo_lats, 
                        climo_lons) = climo_lower_data(
                           init, climo_string, fstart, fend
                        )

                     if str(analysis).upper() in ['GFSV15','GFSV16','GEFS']:
                        anal_lons, anal_lats = np.meshgrid(anal_x, anal_y)
                     climo_lons, climo_lats = np.meshgrid(climo_lons, 
                                                          climo_lats)

                     if finer_grid:
                        if anal_y[0] > anal_y[-1]:
                           lats_fine = np.arange(
                              anal_y[0], anal_y[-1], -1*finer_grid
                           )
                        else:
                           lats_fine = np.concatenate((
                              np.arange(
                                 anal_y[0], np.min(anal_y), -1*finer_grid
                              ), 
                              np.arange(
                                 np.max(anal_y), anal_y[-1], -1*finer_grid
                              )
                           ))
                        if anal_x[0] < anal_x[-1]:
                           lons_fine = np.arange(
                              anal_x[0], anal_x[-1], finer_grid
                           )
                        else:
                           lons_fine = np.concatenate((
                              np.arange(anal_x[0], np.max(anal_x), finer_grid), 
                              np.arange(np.min(anal_x), anal_x[-1], finer_grid)
                           ))
                        lons_fine, lats_fine = np.meshgrid(lons_fine, 
                                                           lats_fine)
                        anal_temp = diff_data(
                           lons_fine, lats_fine, anal_lons, anal_lats, 
                           anal_temp
                        )
                        use_lons, use_lats = (lons_fine, lats_fine)
                     else:
                        use_lons, use_lats = (anal_lons, anal_lats)

                     gefs_temps = []
                     for p, gefs_ens_mem in enumerate(gefs_ens_mems):
                        try:
                           gefs_mem_temp, gefs_y, gefs_x = ens_lower_data(
                              init, str(model).upper(), gefs_string, fstart, 
                              fend, ens_mem=gefs_ens_mem
                           )
                           if p == 0:
                              gefs_lons, gefs_lats = np.meshgrid(gefs_x, 
                                                                 gefs_y)
                           gefs_mem_temp_interp = diff_data(
                     ...
                     climo_upper_temp_interp = diff_data(
                        use_lons, use_lats, climo_lons, climo_lats, 
                        climo_upper_temp
                     )
                     climo_lower_temp_interp = diff_data(
                        use_lons, use_lats, climo_lons, climo_lats, 
                        climo_lower_temp
                     )

                     gefs_over, gefs_under, gefs_normal = get_ABN(
                        gefs_temps, climo_upper_temp_interp, 
                        climo_lower_temp_interp
                     )
                     anal_over, anal_under, anal_normal = get_ABN(
                        np.array([anal_temp]), climo_upper_temp_interp, 
                        climo_lower_temp_interp
                     )
                     
                     gefs_heatmap = np.multiply(
                        np.nanmax(
                           [gefs_under, gefs_normal, gefs_over], 
                           axis=0
                        ), 
                        np.argmax(
                           [gefs_under, gefs_normal, gefs_over], 
                     ...
                     anal_heatmap = np.multiply(
                        np.nanmax(
                           [anal_under, anal_normal, anal_over], 
                           axis=0 
                        ), 
                        np.argmax(
                           [anal_under, anal_normal, anal_over], 
                     ...
                     mgefs_over, mgefs_under, mgefs_normal = [
                        np.ma.MaskedArray(gefs_prob, mask=mask) 
                        for gefs_prob in [gefs_over, gefs_under, gefs_normal]
                     ]
                     manal_over, manal_under, manal_normal = [
                        np.ma.MaskedArray(anal_prob, mask=mask) 
                        for anal_prob in [anal_over, anal_under, anal_normal]
                     ]
                     ...
                     if str(domain).upper() == 'OTHER':
                        plot_temp_probability(
                           num, model, use_lons[1:-1,1:-1], 
                           use_lats[1:-1,1:-1], gefs_heatmap[1:-1,1:-1], 
                           fstart, fend, BSS_under=BSS_under, 
                           BSS_over=BSS_over, BSS_normal=BSS_normal, init=init, 
                           domain=domain, parallels=parallels, merid=merid, 
                           lonrange=lon_range, latrange=lat_range, 
                           figsize=figsize
                        )
                     else:
                        plot_temp_probability(
                     ...
                     if str(domain).upper() == 'OTHER':
                        plot_temp_probability(
                           num, analysis, use_lons[1:-1,1:-1], 
                           use_lats[1:-1,1:-1], anal_heatmap[1:-1,1:-1], 
                           fstart, fend, analysis=True, init=init, 
                           domain=domain, parallels=parallels, merid=merid, 
                           lonrange=lon_range, latrange=lat_range, 
                           figsize=figsize
                        )
